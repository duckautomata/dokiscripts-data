...
def get_audio(url, download_type, channel):
    ...
    # yt-dlp will create the directories. We use forward slashes,
    # as yt-dlp handles them correctly on all operating systems.
    output_template = (
        f"{BASE_DIR}/{channel}/%(upload_date)s - {download_type} - "
        f"%(title)s - [%(id)s].%(ext)s"
    )

    # 2. Build the base command as a list of arguments
    command = [
        YT_DLP_CMD,
        ...
        "--sleep-requests", "1",
        "--sleep-interval", "15",
        url  # The URL is the final argument
    ]

    # 3. Conditionally add/remove arguments based on URL
    if "twitch.tv" in url.lower():
        print(f"Downloading from Twitch (no thumbnail): {channel} - {download_type}")
    else:
        print(f"Downloading (with thumbnail): {channel} - {download_type}")
        # Add --write-thumbnail. We insert it after --match-filter
        command.insert(6, "--write-thumbnail")

    # 4. Run the command
    try:
        # We don't use check=True because the original script uses --ignore-errors
